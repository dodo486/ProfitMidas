...
public class CmtyFuture_종가Feed extends Feed implements DBData {

    public CmtyFuture_종가Feed(FeedTRCode trCode, byte[] packet) {
        super(trCode, packet);
    }
    ...
    public byte[] getCodeByte() {
        return CmtyFuture_종가.isinCode.parser().parseByte(rawPacket);
    }
    ...
    public Integer getPrice() {
        return CmtyFuture_종가.price.parser().parseIntWithLeadingSign(rawPacket);
    }

    public Integer getAskPrice() {
        return CmtyFuture_종가.askPrice.parser().parseIntWithLeadingSign(rawPacket);
    }

    public Integer getAskAmount() {
        return CmtyFuture_종가.askAmount.parser().parseInt(rawPacket);
    }

    public Integer getBidPrice() {
        return CmtyFuture_종가.bidPrice.parser().parseIntWithLeadingSign(rawPacket);
    }

    public Integer getBidAmount() {
        return CmtyFuture_종가.bidAmount.parser().parseInt(rawPacket);
    }

    public Integer getTotalContractAmount() {
        return CmtyFuture_종가.totalContractAmount.parser().parseInt(rawPacket);
    }

    public Long getTotalContractPrice() {
        return CmtyFuture_종가.totalContractPrice.parser().parseLong(rawPacket);
    }
    ...
    public DBObject toDBObject() {
        ...
        String isinCode = CmtyFuture_종가.isinCode.parser().parseStr(line);
//        String priceStr =  CmtyFuture_종가.price.parser().parseStr(line) ;
//        if ( CmtyFuture_종가.priceSign.parser().parseStr(line).equals("-"))
//            priceStr = "-" + priceStr;
//
//        Integer price = null;
//        try {
...
//        }
        Integer price = CmtyFuture_종가.price.parser().parseIntWithLeadingSign(rawPacket);
        DBObject ob = new BasicDBObject();
        ob.put("isinCode", isinCode);
        if (price != null) {
            ob.put("price", price);
        }
        Integer totalContractAmount = CmtyFuture_종가.totalContractAmount.parser().parseInt(rawPacket);
        Long totalContractPrice = CmtyFuture_종가.totalContractPrice.parser().parseLong(rawPacket);
    ...
    public BidAskCore getFullBidAsk(BidAskFactory factory) {
        try {
            ...
            for (int i = 0; i < DERIV_MAX_BIDASK_DEPTH; i++) {
                fullBidAsk.askAmount[i] = CmtyFuture_종가.askAmount.plus(i * size).parseInt(rawPacket);
                if (fullBidAsk.askAmount[i] == 0)
                    break;
                fullBidAsk.askPrice[i] = CmtyFuture_종가.askPrice.plus(i * size).parseIntWithLeadingSign(rawPacket);
            }
            for (int i = 0; i < DERIV_MAX_BIDASK_DEPTH; i++) {
                fullBidAsk.bidAmount[i] = CmtyFuture_종가.bidAmount.plus(i * size).parseInt(rawPacket);
                if (fullBidAsk.bidAmount[i] == 0)
                    break;
                fullBidAsk.bidPrice[i] = CmtyFuture_종가.bidPrice.plus(i * size).parseIntWithLeadingSign(rawPacket);
            }
