...
public class KQOption_종가Feed extends Feed implements DBData {

    public KQOption_종가Feed(FeedTRCode trCode, byte[] packet) {
        super(trCode, packet);
    }

    public byte[] getCodeByte() {
        return KQOption_종가.isinCode.parser().parseByte(rawPacket);
    }
    ...
    public Integer getPrice() {
        return KQOption_종가.price.parser().parseInt(rawPacket);
    }

    public Integer getAskPrice() {
        return KQOption_종가.askPrice.parser().parseInt(rawPacket);
    }

    public Integer getAskAmount() {
        return KQOption_종가.askAmount.parser().parseInt(rawPacket);
    }

    public Integer getBidPrice() {
        return KQOption_종가.bidPrice.parser().parseInt(rawPacket);
    }

    public Integer getBidAmount() {
        return KQOption_종가.bidAmount.parser().parseInt(rawPacket);
    }

    public Integer getTotalContractAmount() {
        return KQOption_종가.totalContractAmount.parser().parseInt(rawPacket);
    }

    public Long getTotalContractPrice() {
        return KQOption_종가.totalContractPrice.parser().parseLong(rawPacket) * 1000;
    }
    ...
    public DBObject toDBObject() {
        ...
        String isinCode = KQOption_종가.isinCode.parser().parseStr(line);
        int price = KQOption_종가.price.parser().parseInt(line);

        DBObject ob = new BasicDBObject();
        ob.put("isinCode", isinCode);
        ob.put("price", price);

        Integer totalContractAmount = KQOption_종가.totalContractAmount.parser().parseInt(rawPacket);
        Long totalContractPrice = KQOption_종가.totalContractPrice.parser().parseLong(rawPacket) * 1000;
...
//        1 + 5 + 6 = 12
        ...
        for (int i = 0; i < DERIV_MAX_BIDASK_DEPTH; i++) {
            fullBidAsk.askAmount[i] = KQOption_종가.askAmount.plus(i * size).parseInt(rawPacket);
            if (fullBidAsk.askAmount[i] == 0)
                break;
            fullBidAsk.askPrice[i] = KQOption_종가.askPrice.plus(i * size).parseInt(rawPacket);
        }
        for (int i = 0; i < DERIV_MAX_BIDASK_DEPTH; i++) {
            fullBidAsk.bidAmount[i] = KQOption_종가.bidAmount.plus(i * size).parseInt(rawPacket);
            if (fullBidAsk.bidAmount[i] == 0)
                break;
            fullBidAsk.bidPrice[i] = KQOption_종가.bidPrice.plus(i * size).parseInt(rawPacket);
        }
