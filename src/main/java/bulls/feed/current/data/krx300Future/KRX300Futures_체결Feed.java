...
public class KRX300Futures_체결Feed extends Feed implements 체결Feed, InfluxDBData {

    public KRX300Futures_체결Feed(FeedTRCode trCode, byte[] packet) {
        super(trCode, packet);
    }
    ...
    public byte[] getCodeByte() {
        return KRX300_체결.isinCode.parser().parseByte(rawPacket);
    }

    public String getCode() {
        try {
            return KRX300_체결.isinCode.parser().parseStr(rawPacket);
    ...
    public String getBoardId() {
        return KRX300_체결.boardId.parser().parseStr(rawPacket, "");
    }

    @Override
    public Integer getPrice() {
        return KRX300_체결.price.parser().parseIntWithLeadingSign(rawPacket);
    }

    @Override
    public Integer getAmount() {
        return KRX300_체결.amount.parser().parseInt(rawPacket);
    }

    public LongShort getLongShort() {
        return LongShort.getFromByteValue(KRX300_체결.buySellSign.parser().parseSingleByte(rawPacket));
    }

    public Long getTotalPrice() {
        return KRX300_체결.totalPrice.parser().parseLong(rawPacket) * 1000;
    }

    public Long getTotalAmount() {
        return KRX300_체결.totalAmount.parser().parseLong(rawPacket);
    }
