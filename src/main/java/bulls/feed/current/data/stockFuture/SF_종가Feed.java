...
import bulls.feed.current.parser.stockFuture.SF_종가;
...
public class SF_종가Feed extends Feed implements DBData {
    static int bidAskLength = SF_종가.bidPrice.parser().getLength() + SF_종가.bidPriceSign.parser().getLength() + SF_종가.bidAmount.parser().getLength();

    public SF_종가Feed(FeedTRCode trCode, byte[] packet) {
        super(trCode, packet);
    }
    ...
    public byte[] getCodeByte() {
        return SF_종가.isinCode.parser().parseByte(rawPacket);
    }
    ...
    public Integer getPrice() {
        return SF_종가.price.parser().parseIntWithLeadingSign(rawPacket);
    }

    public Integer getAskPrice() {
        return SF_종가.askPrice.parser().parseIntWithLeadingSign(rawPacket);
    }

    public Integer getAskAmount() {
        return SF_종가.askAmount.parser().parseInt(rawPacket);
    }

    public Integer getBidPrice() {
        return SF_종가.bidPrice.parser().parseIntWithLeadingSign(rawPacket);
    }

    public Integer getBidAmount() {
        return SF_종가.bidAmount.parser().parseInt(rawPacket);
    }

    public Integer getTotalContractAmount() {
        return SF_종가.totalContractAmount.parser().parseInt(rawPacket);
    }

    public Long getTotalContractPrice() {
        return SF_종가.totalContractPrice.parser().parseLong(rawPacket);
    }
    ...
    public DBObject toDBObject() {
        ...
        String isinCode = SF_종가.isinCode.parser().parseStr(line);
//        String priceStr =  SF_종가.price.parser().parseStr(line) ;
//        if ( SF_종가.priceSign.parser().parseStr(line).equals("-"))
//            priceStr = "-" + priceStr;
//
//        Integer price = null;
//        try {
...
//        }
        Integer price = SF_종가.price.parser().parseIntWithLeadingSign(rawPacket);
        DBObject ob = new BasicDBObject();
        ob.put("isinCode", isinCode);
        if (price != null) {
            ob.put("price", price);
        }
        Integer totalContractAmount = SF_종가.totalContractAmount.parser().parseInt(rawPacket);
        Long totalContractPrice = SF_종가.totalContractPrice.parser().parseLong(rawPacket);
    ...
    public BidAskCore getFullBidAsk(BidAskFactory factory) {
        try {
            ...
            for (int i = 0; i < DERIV_STOCK_MAX_BIDASK_DEPTH; i++) {
                fullBidAsk.askAmount[i] = SF_종가.askAmount.plus(i * size).parseInt(rawPacket);
                if (fullBidAsk.askAmount[i] == 0)
                    break;
                fullBidAsk.askPrice[i] = SF_종가.askPrice.plus(i * size).parseIntWithLeadingSign(rawPacket);
            }
            for (int i = 0; i < DERIV_STOCK_MAX_BIDASK_DEPTH; i++) {
                fullBidAsk.bidAmount[i] = SF_종가.bidAmount.plus(i * size).parseInt(rawPacket);
                if (fullBidAsk.bidAmount[i] == 0)
                    break;
                fullBidAsk.bidPrice[i] = SF_종가.bidPrice.plus(i * size).parseIntWithLeadingSign(rawPacket);
            }
