...
public class SF_종목정보Feed extends Feed implements DBData, 종목정보Feed, SQLDBDataConvertible {
    DBObject ob = new BasicDBObject();

    public SF_종목정보Feed(FeedTRCode trCode, byte[] packet) {
        super(trCode, packet);
        ...
        double 기준가 = 선물_종목정보.기준가.parser().parseDoubleInsertDot(rawPacket, 2);

        int 잔존일수 = 선물_종목정보.잔존일수.parser().parseInt(rawPacket);
        double 전일종가 = 선물_종목정보.전일종가.parser().parseFloatWithLeadingSign(rawPacket, 2);
        double 전일시가 = 선물_종목정보.전일시가.parser().parseFloatWithLeadingSign(rawPacket, 2);
        double 전일고가 = 선물_종목정보.전일고가.parser().parseFloatWithLeadingSign(rawPacket, 2);
        double 전일저가 = 선물_종목정보.전일저가.parser().parseFloatWithLeadingSign(rawPacket, 2);
        ...
        double 가격제한1단계상한가 = 선물_종목정보.가격제한1단계상한가.parser().parseFloatWithLeadingSign(rawPacket, 2);
        double 가격제한1단계하한가 = 선물_종목정보.가격제한1단계하한가.parser().parseFloatWithLeadingSign(rawPacket, 2);
        double 가격제한2단계상한가 = 선물_종목정보.가격제한2단계상한가.parser().parseFloatWithLeadingSign(rawPacket, 2);
        double 가격제한2단계하한가 = 선물_종목정보.가격제한2단계하한가.parser().parseFloatWithLeadingSign(rawPacket, 2);
        double 가격제한3단계상한가 = 선물_종목정보.가격제한3단계상한가.parser().parseFloatWithLeadingSign(rawPacket, 2);
        double 가격제한3단계하한가 = 선물_종목정보.가격제한3단계하한가.parser().parseFloatWithLeadingSign(rawPacket, 2);
        ...
        if(TempConf.UPDATE_FROM_FEED) {
            ...
            futuresInfo.priceDivider = (int) Math.pow(10, priceDecimalPoint);
            ;
    ...
    public SQLDBData[] getSQLDBDataArray() {
        try {
            ...
            Double 거래단위 = 선물_종목정보.거래단위.parser().parseDoubleInsertDot(rawPacket, 8);
            Double ATMPrice = 선물_종목정보.ATMPrice.parser().parseFloatWithLeadingSign(rawPacket, 2);
